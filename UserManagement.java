...
public class UserManagement {
	...
	public static void Write(ArrayList<ArrayList<String>> users) throws IOException{
		BufferedWriter buff=new BufferedWriter(new FileWriter("C:/Users/rmonroe/Documents/Userfile.txt"));
	 	
	 	//for loop that writes the text to the file in the specific format
        for(int i=0; i<users.size(); i++){
        	buff.write(users.get(i).get(0) + " " + users.get(i).get(1));
            buff.newLine();
        }
		...
		loop: while(exitnum!=5){
			...
			switch(mAnswer){
			    ...
			    case(3):
					...
					switch(mAnswer){
					case(1):
						System.out.println("Please enter the user name you'd like to update");
						Scanner unamescan=new Scanner (System.in);
						String username=unamescan.nextLine();
						for(int l=0; l<users.size(); l++){
							if(users.get(l).get(0).equals(username)){
								System.out.println("What would you like to update the user name to?");
								String newUserName=unamescan.nextLine();
								users.get(l).remove(0);
								users.get(l).add(0, newUserName);
							}
						}
						//Adds the activity to the user log report
			    ...
			    case(4):
					...
					for(int i=0; i<users.size(); i++){
						if(users.get(i).get(0).equals(name) && users.get(i).get(1).equals(Main.Encryption(password))){
							users.remove(i);
						}
					}
					//Adds the activity to the user log report
					activity="Deleted a user: " + name;
					Main.TrackActivity(activity);
			    	break;
